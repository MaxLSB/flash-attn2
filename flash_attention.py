...
class FlashAttention(torch.autograd.Function):
    ...
    def backward(ctx, dO):
        ...
        # NUM_WARPS -> Number of warps (groups of 32 threads) per GPU block 
        # NUM_STAGES -> Number of software pipelining stages for prefetching data
        # BLOCK_SIZE_MICRO -> ??
        # BLOCK_SIZE_MACRO ??
        BATCH_SIZE, NUM_HEADS, SEQ_LEN, HEAD_DIM = Q.shape
...
def _attn_fwd_inner(
    ...
    for start_kv in range(lower, higher, BLOCK_SIZE_KV):
        ...
        if STAGE == 2:
            ...
            mask = q_offsets[:, None] >= (start_kv + key_values_offsets[None, :])
...
# Triton kernel for the backward pass to compute dV and dK
@triton.jit
def _attn_bwd_dk_dv(
    Q,
    K,
    ...
    # Load K and V in SRAM -> (BLOCK_KV, HEAD_DIM)
    V_block = tl.load(V + kv_offsets[:, None] * stride_seq + dim_offsets[None, :] * stride_dim)
    K_block = tl.load(K + kv_offsets[:, None] * stride_seq + dim_offsets[None, :] * stride_dim)
    
    q_offsets = tl.arange(0, BLOCK_Q)
    
    # We access Q as a transposed matrix -> (HEAD_DIM, BLOCK_Q)
    # We point to the first BLOCK_Q rows of Q for both q_T and dO pointers, inside the for loop we will move forward by BLOCK_Q rows at each iteration
    ...
    for _ in range(num_steps):
        ...
        q_offsets = curr_q + tl.arange(0, BLOCK_Q)
        m = tl.load(L + q_offsets)
        
        # K(Q^T) = S^T
        S_T_block = softmax_factor * tl.dot(K_block, Q_T_block)
        # softmax with logsumexp trick
        P_T_block = tl.math.exp(S_T_block - m[:, None])
        
        if STAGE == 3:
            # For causal attention
            # Mask is TRUE for values that do not need to be masked -> (BLOCK_KV, BLOCK_Q)
        ...
        # Update the pointers
        curr_q += BLOCK_Q
        Q_T_ptrs += BLOCK_Q * stride_seq
        dO_ptrs += BLOCK_Q * stride_seq

    # Store the computed dK and dV blocks
    dV_block_ptrs = dV + kv_offsets[:, None] * stride_seq + dim_offsets[None, :] * stride_dim
    tl.store(dV_block_ptrs, dV_block)
    
    dK_block_ptrs = dK + kv_offsets[:, None] * stride_seq + dim_offsets[None, :] * stride_dim
    tl.store(dK_block_ptrs, dK_block)


# Triton kernel for the backward pass to compute dQ
@triton.jit
def _attn_bwd_dq(
    Q,
    K,
    ...
    index_batch = index_batch_head // NUM_HEADS
    index_head = index_batch_head % NUM_HEADS
    offset_batch_head = (index_batch * stride_batch + index_head * stride_head).to(tl.int64)
    offset_batch_head_seq = (offset_batch_head * SEQ_LEN).to(tl.int64)
    ...
    Q_block = tl.load(Q + q_offsets[:, None] * stride_seq + dim_offsets[None, :] * stride_dim)
    ...
    for _ in range(num_steps):
        ...
        if STAGE == 3:
            # For Causal Attention
            kv_offsets = curr_kv + tl.arange(0, BLOCK_KV)
            mask_block = q_offsets[:, None] >= kv_offsets[None, :]
        ...
        # Update the pointers
        curr_kv += BLOCK_KV
        K_T_ptrs += BLOCK_KV * stride_seq
        V_T_ptrs += BLOCK_KV * stride_seq

    dQ_block_ptrs = dQ + q_offsets[:, None] * stride_seq + dim_offsets[None, :] * stride_dim
